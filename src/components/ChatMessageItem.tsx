...
export const ChatMessageItem: React.FC<ChatMessageItemProps> = ({
  ...
  // Auto-collapse thinking when answer text is present and thinking has finished
  useEffect(() => {
    if (!message.isThinking && message.text) {
      ...
      >
        <div className="w-full max-w-3xl mx-auto px-3 sm:px-4 flex gap-2.5 sm:gap-4 items-start">
          ...
          <div className="shrink-0 mt-0.5">
            {isUser ? (
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 border border-slate-200 shadow-2xs">
                U
              </div>
            ) : (
          ...
          <div className={`flex-1 min-w-0 ${isUser ? 'space-y-2 pt-0.5' : 'space-y-3 bg-slate-50/90 p-3.5 sm:p-5 md:p-6 rounded-2xl border border-slate-100 shadow-2xs'}`}>
            ...
            {/* Live AI Thinking & Reasoning Process */}
            {!isUser && (message.isThinking || message.thinkingText) && (
              <div className="mb-3 rounded-2xl border border-indigo-200/80 bg-gradient-to-br from-indigo-50/90 via-slate-50 to-purple-50/60 p-3 shadow-2xs transition-all">
                <button
                  type="button"
                  onClick={() => setIsThinkingExpanded(!isThinkingExpanded)}
                  className="w-full flex items-center justify-between text-left text-xs font-semibold text-indigo-950 select-none cursor-pointer group"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-lg bg-indigo-600/10 text-indigo-600 flex items-center justify-center border border-indigo-200/60">
                      <Brain className={`w-3.5 h-3.5 ${message.isThinking ? 'animate-pulse text-indigo-600' : 'text-slate-600'}`} />
                    </div>
                    <span>
                      {message.isThinking ? (
                        <span className="flex items-center gap-1.5 font-bold text-indigo-900">
                          <span>Thinking Process</span>
                          <span className="font-mono text-[11px] text-indigo-600 font-normal">({thinkingElapsed}s)</span>
                        ...
                        <span className="text-slate-700 font-medium">
                          Thought Process {message.thinkingDurationMs ? `(${(message.thinkingDurationMs / 1000).toFixed(1)}s)` : ''}
                        </span>
                      )}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 text-[11px] text-indigo-600 group-hover:text-indigo-800 font-medium">
                ...
                {isThinkingExpanded && (
                  <div className="mt-2.5 pt-2.5 border-t border-indigo-100/90 text-xs text-slate-600 font-mono leading-relaxed whitespace-pre-wrap max-h-56 overflow-y-auto pr-1">
              ...
              {isUser ? (
                ...
                <div>
                  {message.text ? (
                    ...
                    <div className="flex items-center gap-2 text-indigo-600 text-sm py-1 animate-pulse">
                      <Sparkles className="w-4 h-4 animate-spin text-indigo-600" />
                      <span>Echo is formulating fast response...</span>
                    </div>
                  ) : null}
            ...
            {showTranslation && translatedText && (
              <div className="mt-3 p-3.5 rounded-2xl bg-indigo-50/70 border border-indigo-200/80 text-slate-800 text-xs sm:text-sm animate-fadeIn">
                <div className="flex items-center justify-between font-bold text-indigo-950 mb-1.5 flex-wrap gap-1">
                  <span className="flex items-center gap-1.5 text-xs text-indigo-700">
                    <Languages className="w-3.5 h-3.5" />
                    {detectedIsUrdu ? 'English Translation' : 'اردو ترجمہ (Urdu Translation)'}
                  </span>
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={handleSpeakTranslation}
                      className={`text-xs px-2.5 py-0.5 rounded-md border flex items-center gap-1 font-semibold transition-all cursor-pointer ${
                ...
                <div
                  className="leading-relaxed whitespace-pre-wrap font-sans text-slate-900"
                  dir={!detectedIsUrdu ? 'rtl' : 'ltr'}
                >
                  {translatedText}
                </div>
              </div>
            ...
            {/* Action Toolbar (Copy, Speak Voice, Translate & Speak, Regenerate, Edit) */}
            {!message.isStreaming && !message.error && message.text && (
              <div className="flex items-center gap-1.5 pt-2 opacity-90 group-hover:opacity-100 transition-opacity border-t border-slate-100/80 flex-wrap">
                {/* Voice Speech (TTS) Button - Auto Urdu or English in Real Studio Voice */}
                <button
                  ...
                  title={
                    isSpeakingThis
                      ? 'Stop voice speech'
                      : `Listen in real human voice (${detectedIsUrdu ? 'Urdu / اردو' : 'English'})`
                  }
                >
                  {isSpeakingThis ? (
                    <>
                    ...
                    <>
                      ...
                      <span>{detectedIsUrdu ? 'سنیں (Listen Urdu)' : 'Listen (Real Voice)'}</span>
                    </>
                  )}
                </button>

                {/* Translate & Speak Voice Button */}
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-all cursor-pointer border ${
                    isSpeakingTranslation
                      ...
                      : 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50/60 border-slate-200/60'
                  }`}
                  title={
                    detectedIsUrdu
                      ? 'Translate to English & Speak in Real Voice'
                      : 'اردو میں ترجمہ اور آواز سنیں (Translate to Urdu & Speak in Real Voice)'
                  }
                >
                  {isTranslating ? (
                    <>
                    ...
                    <>
                      <VolumeX className="w-3.5 h-3.5 animate-bounce" />
                      <span>Stop Translation Voice</span>
                    </>
                  ) : (
                    <>
                      <Languages className="w-3.5 h-3.5 text-indigo-600" />
                      <span>{detectedIsUrdu ? 'Speak in English' : 'اردو آواز (Urdu Voice)'}</span>
                    </>
                  )}
                </button>

                {/* Toggle View Translation if translated */}
                {translatedText && !showTranslation && (
                  <button
                    onClick={() => setShowTranslation(true)}
                    className="flex items-center gap-1 px-2 py-1 rounded-md text-xs text-indigo-600 hover:bg-indigo-50 border border-indigo-200/60"
                  >
                ...
                <button
                  ...
                  onClick={handleCopy}
                  className="flex items-center gap-1 px-2 py-1 rounded-md text-xs text-slate-500 hover:text-indigo-600 hover:bg-slate-100 transition-colors"
                  title="Copy response"
                >
                  {copied ? (
                    <>
                ...
                {onReply && (
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 rounded-md text-xs text-slate-500 hover:text-indigo-600 hover:bg-slate-100 transition-colors cursor-pointer"
