...
export const GenerateImageModal: React.FC<GenerateImageModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="w-full max-w-3xl bg-white border border-slate-200/90 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] animate-fadeIn">
        {/* Header with Model Identifier & Tab Selector */}
        <div className="px-5 py-3.5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/90">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-900 border border-amber-300/80 flex items-center gap-1 shadow-2xs">
                  <span>🍌</span>
                  <span>Nano Banana (Gemini Image)</span>
                </span>
              </div>
              <p className="text-xs text-slate-500">
                Text-to-image creation and text-guided image editing with Nano Banana
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-center">
            ...
            <button
              onClick={onClose}
              className="p-1.5 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-100 transition-colors ml-1 cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5 flex-1">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 text-xs shadow-2xs">
            <div className="flex items-center gap-2">
              <span className="text-base">🍌</span>
              <div>
                <span className="font-bold text-amber-900">
                  Daily Nano Banana Points:
                </span>{' '}
                <span className="text-amber-800">
                  <strong className="font-semibold text-slate-900">{dailyQuota.remaining} of {MAX_DAILY_IMAGES}</strong> images remaining today
                </span>
                <span className="text-[11px] text-amber-700/80 block sm:inline sm:ml-1">
                  (5 points/day for all Google accounts • resets midnight)
                </span>
              </div>
            </div>
                ...
                <div
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all ${
                    i < dailyQuota.used
                      ...
                      : 'bg-emerald-500 shadow-xs ring-2 ring-emerald-200/60'
                  }`}
